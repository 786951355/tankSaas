...
import com.tanker.resmodule.widget.SimpleMonthView;
...
public class SimpleMonthAdapter extends RecyclerView.Adapter<SimpleMonthAdapter.ViewHolder> implements SimpleMonthView.OnDayClickListener {
    ...
    public SimpleMonthAdapter(Context context, DatePickerController datePickerController, TypedArray typedArray) {
        ...
        selectedDays.setFirst(new CalendarDay());
//        selectedDays.setLast(new CalendarDay());
        mContext = context;
        this.typedArray = typedArray;
        mController = datePickerController;
        minCalendar = Calendar.getInstance();
        maxCalendar = Calendar.getInstance();
        minCalendar.setTime(mController.getMinDate());
        maxCalendar.setTime(mController.getMaxDate());
        firstMonth = typedArray.getInt(R.styleable.DayPickerView_firstMonth, minCalendar.get(Calendar.MONTH));
        init();
    }
    ...
    public void onBindViewHolder(ViewHolder viewHolder, int position) {
        ...
        day = maxCalendar.get(Calendar.DAY_OF_MONTH);
        month = (firstMonth + (position % MONTHS_IN_YEAR)) % MONTHS_IN_YEAR;
        year = position / MONTHS_IN_YEAR + minCalendar.get(Calendar.YEAR) + ((firstMonth + (position % MONTHS_IN_YEAR)) / MONTHS_IN_YEAR);
        ...
        drawingParams.put(SimpleMonthView.VIEW_PARAMS_DAY, day);
        drawingParams.put(SimpleMonthView.VIEW_PARAMS_MAX_YEAR, maxCalendar.get(Calendar.YEAR));
        drawingParams.put(SimpleMonthView.VIEW_PARAMS_MAX_MONTH, maxCalendar.get(Calendar.MONTH));
        drawingParams.put(SimpleMonthView.VIEW_PARAMS_MAX_DAY, maxCalendar.get(Calendar.DAY_OF_MONTH));
        drawingParams.put(SimpleMonthView.VIEW_PARAMS_WEEK_START, minCalendar.getFirstDayOfWeek());
    ...
    public int getItemCount() {
        ...
        int maxMonth = maxCalendar.get(Calendar.MONTH) + 1;
        int minMonth = minCalendar.get(Calendar.MONTH) + 1;
        int itemCount;
        if (maxYear < minYear || (maxYear == minYear && maxMonth < minMonth)) {
            ...
            itemCount = (maxCalendar.get(Calendar.YEAR) - minCalendar.get(Calendar.YEAR)) * MONTHS_IN_YEAR
                    + maxMonth - minMonth + 1;
        }
        return itemCount;
    }
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(View itemView, SimpleMonthView.OnDayClickListener onDayClickListener) {
            ...
            simpleMonthView.setMaxDate(maxCalendar.getTime());
        }
    }
    ...
    public static class CalendarDay implements Serializable {
        ...
        public String toString() {
            return DateUtils.format(year, month + 1, day, DateUtils.DATE_FORMAT_YMD);
        }
    }
